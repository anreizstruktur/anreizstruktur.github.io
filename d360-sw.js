 ...
 *
 */
var D360LoggerSeverity = Object.freeze({
    NONE: "NONE",
    ERROR: "ERROR",
    WARNING: "WARNING",
    INFO: "INFO",
...
var InitialConfigurationController = function() {
    this.indexedDBHandle = null;
    this.indexedDBInitialized = false;
    this.pendingConfigurationUpdate = null;
}

InitialConfigurationController.prototype.createStoreTx = function() {
    var indexedDBTx= this.indexedDBHandle.transaction("configurationData", "readwrite");
    return indexedDBTx.objectStore('configurationData');
}

InitialConfigurationController.prototype.init = function() {
    var controller = this;

    var indexedDBHandle = null;
    var indexDBOpener = indexedDB.open("D360SWDB", 1);
    indexDBOpener.onerror = function(event) {
        logger.log(D360LoggerSeverity.ERROR, 'opening error', 'indexeddb', event)
    };

    // Create the schema
    indexDBOpener.onupgradeneeded = function() {
        var db = indexDBOpener.result;
        var store = db.createObjectStore("configurationData", {keyPath: "id"});
    };

    indexDBOpener.onsuccess = function(event) {
        controller.indexedDBHandle = indexDBOpener.result;
        var storeTx = controller.createStoreTx();

        var loadInitialConfigurationHandler = storeTx.get(1);
        loadInitialConfigurationHandler.onerror = function() {
            logger.log(D360LoggerSeverity.DEBUG, 'no initial configuration', 'indexeddb', event)
        }
        loadInitialConfigurationHandler.onsuccess = function() {
            var result = loadInitialConfigurationHandler.result;
            if(result) {
                logger.log(D360LoggerSeverity.DEBUG, 'initial configuration found', 'indexeddb', result)
                if(result.pushConfig) {
                    D360SimpleEventEmitter.pushConfig = result.pushConfig;
                }
                if(result.eventTemplate) {
                    D360SimpleEventEmitter.eventTemplate = result.eventTemplate;
                }
                if(result.sessionId) {
                    D360SimpleEventEmitter.sessionId = result.sessionId;
                }
                if(result.externalUserId) {
                    D360SimpleEventEmitter.externalUserId = result.externalUserId;
                }
                if(result.deviceId) {
                    D360SimpleEventEmitter.deviceId = result.deviceId;
                }
            } else {
                logger.log(D360LoggerSeverity.DEBUG, 'initial configuration empty', 'indexeddb', loadInitialConfigurationHandler)
            }

            if(controller.pendingConfigurationUpdate) {
                controller.updateConfiguration(controller.pendingConfigurationUpdate);
            }
            logger.log(D360LoggerSeverity.DEBUG, 'init mode completed', 'indexeddb')

            controller.indexedDBInitialized = true;
            D360SimpleEventEmitter.flushEvents();

        }
    };
}

InitialConfigurationController.prototype.updateConfiguration = function(configuration) {
    var storeTx = this.createStoreTx();
    configuration.id = 1;
    var updaterRequest = storeTx.put(configuration);
    logger.log(D360LoggerSeverity.DEBUG, 'update configuration', 'indexeddb', configuration)
    updaterRequest.onsuccess = function() {
        logger.log(D360LoggerSeverity.DEBUG, 'initial configuration updated', 'indexeddb', updaterRequest)
    }
    updaterRequest.onerror = function(e) {
        logger.log(D360LoggerSeverity.DEBUG, 'initial configuration update error', 'indexeddb', e)
    }
}

InitialConfigurationController.prototype.queueUpdate = function(configuration) {
    if(this.indexedDBInitialized) {
        this.updateConfiguration(configuration);
    } else {
        this.pendingConfigurationUpdate = configuration;
    }
}

var initialConfiguration = new InitialConfigurationController();
initialConfiguration.init();



D360SimpleEventEmitter.isReady = false;
D360SimpleEventEmitter.queuedEvents = [];
D360SimpleEventEmitter.generateUUID = function() {
...
D360SimpleEventEmitter.queueEvent = function(event, props, sessionId, callback) {
    if(!D360SimpleEventEmitter.isReady) {
        D360SimpleEventEmitter.queuedEvents.push([event, props, sessionId, callback]);
        logger.log(D360LoggerSeverity.DEBUG, 'event queued', 'D360SimpleEventEmitter', event)
    } else {
        D360SimpleEventEmitter.sendEvent(event, props, sessionId, callback);
    }
};
D360SimpleEventEmitter.flushEvents = function() {
    logger.log(D360LoggerSeverity.DEBUG, 'event flush', 'D360SimpleEventEmitter')
    D360SimpleEventEmitter.isReady = true;
    for(var k in D360SimpleEventEmitter.queuedEvents) {
        var event = D360SimpleEventEmitter.queuedEvents[k];
        D360SimpleEventEmitter.sendEvent(event[0], event[1], event[2], event[3])
    }
};
D360SimpleEventEmitter.sendEvent = function (event, props, sessionId, callback) {
    if(sessionId) {
        D360SimpleEventEmitter.sessionId = sessionId;
    }
    if(!D360SimpleEventEmitter.sessionId || typeof(D360SimpleEventEmitter.sessionId) == 'undefined') {
        D360SimpleEventEmitter.sessionId = D360SimpleEventEmitter.generateUUID();
    }
    var payload = D360SimpleEventEmitter.eventTemplate;
    payload.events[0].id = D360SimpleEventEmitter.generateUUID();
    payload.events[0].timestamp = (new Date()).getTime().toString();
    payload.events[0].name = event;
    payload.events[0].properties = props;
    payload.events[0].session_id = this.sessionId;
    if(D360SimpleEventEmitter.externalUserId) {
        payload.events[0].external_user_id = this.externalUserId+"";
    }

    var content = JSON.stringify(payload);

    var hMacCalculator = new jsSHA("SHA-512", "TEXT")
    hMacCalculator.setHMACKey(D360SimpleEventEmitter.pushConfig.apiSecret, "HEX");
    hMacCalculator.update(content);
    ...
    var headers = {
        ...
        "D360-Signature": hMacCalculator.getHMAC("B64")
    }

    if(this.deviceId) {
        if(D360SimpleEventEmitter.deviceId.device_id != undefined)
            headers['D360-Device-Id'] = D360SimpleEventEmitter.deviceId.device_id;
    ...
    fetch('https://rt.360dialog.io/xray/events/360dialog/sdk/v1', {
        ...
        .then(json)
        .then(function (data) {
            logger.log(D360LoggerSeverity.DEBUG, 'event backend response', 'D360SimpleEventEmitter', data)
            if (callback) {
                callback(data);
            }
        })
        .catch(function (error) {
            logger.log(D360LoggerSeverity.ERROR, 'event backend request failed', 'D360SimpleEventEmitter', error)
        })
}

var D360PushGateway = {};
D360PushGateway.sendMessageToClient = function(client, command, message) {
    return new Promise(function(resolve, reject){
        ...
        client.postMessage({command: command, properties: message}, [channel.port2]);
    });
}
D360PushGateway.sendMessageToAll = function(command, message) {
    logger.log(D360LoggerSeverity.DEBUG, 'sending message to all', 'D360PushGateway', [command, message])
    clients.matchAll({includeUncontrolled: true, type: 'window'}).then(function(iClient) {
        iClient.forEach(function(client) {
            D360PushGateway.sendMessageToClient(client, command, message).then(function(m) {console.log("SW Received Message: "+m); });
        });
    })
}
...
self.addEventListener('message', function (event) {
    ...
    if (eventData.command == "configuration") {
        logger.log(D360LoggerSeverity.DEBUG, 'received configuration', 'sw-core', eventData)
        if (eventData.message.pushConfig) {
            D360SimpleEventEmitter.pushConfig = eventData.message.pushConfig;
        }
        if (eventData.message.eventTemplate) {
            D360SimpleEventEmitter.eventTemplate = eventData.message.eventTemplate;
        }
        if (eventData.message.sessionId) {
            D360SimpleEventEmitter.sessionId = eventData.message.sessionId;
        }
        if (eventData.message.externalUserId && eventData.message.externalUserId!=undefined) {
            D360SimpleEventEmitter.externalUserId = eventData.message.externalUserId;
        }
        if (eventData.message.deviceId) {
            D360SimpleEventEmitter.deviceId = eventData.message.deviceId;
        } else {
            D360SimpleEventEmitter.sendEvent('d360_register', {v2_app_instance_id: D360SimpleEventEmitter.pushConfig.id}, null, function(data) {
                if(data.events_status && data.events_status[0] && data.events_status[0].status=='success') {
                    D360SimpleEventEmitter.deviceId = data.events_status[0].registration_data;
                    D360PushGateway.sendMessageToAll('registeredDevice', D360SimpleEventEmitter.deviceId);
                    logger.log(D360LoggerSeverity.DEBUG, 'successful backend registration', 'sw-core', data)
                } else {
                    logger.log(D360LoggerSeverity.ERROR, 'error during backend registration', 'sw-core')
                }
            });
        }

        var persistentConfiguration = eventData.message;
        initialConfiguration.queueUpdate(persistentConfiguration);

    } else if(eventData.command == "sendEvent") {
        D360SimpleEventEmitter.sendEvent(eventData.message.event, eventData.message.props, eventData.message.sessionId);
    }
});
...
self.addEventListener('push', function (event) {
    ...
    try {
        ...
        // @TODO remove it - it's for test purposes only
        payload = {"tr":{"workflow_id":1,"pid":"592ce48a-3b60-439d-c309-3cc7a04f6257","campaign_id":"1913","action_id":1},"a":{"tr":{"workflow_id":1,"campaign_id":"1913","action_id":1},"trigger":{"when":"now"},"n":"openURL","p":{"u":"https://360dialog.com"}},"n":{"title":"Hi","options":{"image":"https://dummyimage.com/200x100/09f.png","body":"Testing web push! :-)","icon":"https://dummyimage.com/80/09f.png"}}};
    }

    D360SimpleEventEmitter.queueEvent(
        "d360_push_received", {"tr": payload.tr}
    );

    var notificationOptions = payload.n.options;
    notificationOptions.data = {a: payload.a, tr: payload.tr}

    logger.log(D360LoggerSeverity.INFO, 'received event payload', 'sw-push', payload)

    event.waitUntil(
        self.registration.showNotification(payload.n.title ? payload.n.title : 'No title', notificationOptions).then(function() {
            D360SimpleEventEmitter.queueEvent(
                "d360_push_displayed", {"tr": payload.tr}
            );
        })
    )
...
self.addEventListener('notificationclick', function (event) {
    ...
    logger.log(D360LoggerSeverity.INFO, 'notification clicked JS event', 'sw-push', event.notification)

    var payload = event.notification.data.a;
    var payloadTr = event.notification.data.tr;

    D360SimpleEventEmitter.queueEvent(
        "d360_notification_tapped",
        {
            "tr": payloadTr
        }
    );
    ...
    if (clients.openWindow) {
        if(event.notification.data.a.n == 'openURL') {
            return clients.openWindow(event.notification.data.a.p.u);
        }
    }
            ...
            .then(function (clientList) {
                for (var i = 0; i < clientList.length; i++) {
                    ...
                    if (client.url == '/' && 'focus' in client) {
                        client.location = event.notification.data.p.u;
                    }
                }
            })
    );
